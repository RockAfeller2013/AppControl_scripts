...
The script is designed to be executed in the customer’s environment in order to query the App Control server and generate a zip file containing the results. The customer will be required to send the zip file to you in order to upload it to the Symhealth container App web site. The best method for completing this is to create a new Case on the Broadcom Support Portal and have the customer upload it to the case.
